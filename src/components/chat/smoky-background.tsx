...
export function SmokyBackground() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // ─── Diagonal light sweep — animated, moves bottom-right to top-left ───
      const sweepProgress = (time * 0.12) % 2.4;
      // Start from bottom-right, sweep to top-left
      const sweepX = w * (1.2 - sweepProgress);
      const sweepY = h * (1.2 - sweepProgress);
      const sweepGrad = ctx.createLinearGradient(sweepX - w * 0.3, sweepY + h * 0.3, sweepX + w * 0.1, sweepY - h * 0.1);
      if (isDark) {
